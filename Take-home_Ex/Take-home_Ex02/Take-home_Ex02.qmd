...
p2 <- plot_PA_prop(c("Changi", "Downtown Core", "Newton", "Bukit Timah","Clementi", "Sembawang", "Serangoon", "Hougang", "Tampines"))
p2
```
...
#| fig-width: 12.5
#Calling the function to plot proportion population pyramid by the nine planning areas selected by peer
p3 <- plot_PA_prop_bar(c("Changi", "Downtown Core", "Newton", "Bukit Timah","Clementi", "Sembawang", "Serangoon", "Hougang", "Tampines"))
p3
```
...
plot_PA_pop <- function(planning_area) {
  ...
  ggplot(data = sgpop_clean |> 
    ...
    #adding title
    labs(title = "Plot 1. Absolute age-sex pyramid") +
...
plot_PA_prop <- function(planning_area) {
  ...
  ggplot(data = sgpop_clean |> filter(PA %in% planning_area),
    ...
    #adding title
    labs(title = "Plot 2. Proportion age-sex pyramid") +
...
final_plot + plot_annotation(
  title = 'Population sizes differ drastically across Singapore planning areas, however its pattern differences are further revealed when population proportion (%) is plotted',
