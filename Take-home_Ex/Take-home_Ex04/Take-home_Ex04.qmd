...
#### 3.2.1 Singapore Trade Balance in monthly level
...
-   Two heatmaps will be generated, one for the entire Singapore trade balance and one faceted by each country (16 countries)
...
-   As Singapore overall trade balance is mostly positive, a imbalanced color scale needs to be used to ensure the neutral color remains at zero
...
-   ***totalsgtrade_calmap*** will be used to plot the first map displaying the entire Singapore trade balance. Hence, we can utilise ***totalsgtrade*** (refer to @sec-3.1.1.2 ) which already contains the total Singapore import and export values, irrespective of countries. However, the **Month** and **Trade_Balance** variables need to be regenerated
...
-   Base plot is created using `ggplot2::geom_tile()`, specifying the `fill` argument by **Trade_Balance** and `color` (tile border) argument as white. For the second plot, the data is further filtered by ***selected_countries***
...
-   Setting imbalanced color scale using `scale_fill_gradientn()`, specifying that midpoint is 0. The color chosen is colorblind-friendly diverging palette
...
-   Similar insight can be garnered from @sec-3.1.1.3 with Singapore maintaining trade surplus throughout duration, except for brief period in May 2022
-   In 2020: Trade surplus gains momentum from early 2020, peaking in Aug before dropping further towards end of the year. Singapore still maintains healthy trade balance despite Covid-19
-   In 2021: Trade surplus is clearly stronger than 2020, peaking in both Mar and Aug with Covid-19 recovery underway
-   In 2022: Trade surplus is clearly thinning from Mar 2022 onward, with peak in Feb (prior to Russia-Ukraine War) and Sep 2022 (onset of recession), which are lower than previous 2 years
-   No other clear seasonality observed and without trade volume as reference, it is hard to gauge the intensity of trade activities
...
-   Other regional partners: Singapore seems to be maintaining thin surplus with Philippines, which evidently improving from Aug 2022 onwards. It is interesting to see the three peaks of trade surplus with Cambodia happening in Mar and Dec 2021, and Jul 2022.
...
-   Other minor partners: The calendar heatmap still favors bigger trading partners as smaller partners (i.e., Mexico, New Zealand, Canada, Brunei Darussalam) is overshadowed by the rest. No valuable insights

#### 3.2.2 Singapore Trade Export and Import by Selected Countries
