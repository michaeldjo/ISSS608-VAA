...
-   [lease_commence_date]{.underline} is in `<dbl>` format. It needs to be converted to `<chr>`
...
-   Convert the [month]{.underline} variable to date using `as.Date()`. Store the year and month to the respective new variables [year]{.underline} and [month]{.underline} using `dplyr:mutate()`. They can then be converted to string (discrete variables) using `as.character()`. Afterwards, we can filter the year variable to 2022 using `dplyr::filter()`, which is the scope of the study
...
-   The aim is to provide one-glance of levels of uncertainties of median estimate of resale price by town. Hence there is no need to clutter the plot with too much details (i.e., labels and annotations)

-   Unlike one-way ANOVA test, as there is less detail required in visualising uncertainty, it is possible to come up with a single plot
...
-   Define confidence levels in `width` argument of `stat_pointinterval()`. Define the point estimate as `median` and interval as `qi` (quantile confidence intervals). The argument `point_color` also assigns color to the point.
...
One of the most interesting finding in EDA of two continuous variables is the relationship between [resale price per sqm]{.underline} and [remaining lease_years]{.underline}. Unfortunately, without applying any filters, there is very little insight can be garnered from this two-dimensional plot. Additional dimensions could be introduced by applying [town]{.underline} filter, which was found to be statistically significant in Section 3.2. Considerations:
